...
export class Expenseshistory {
 ...
 ngOnInit() {
   this.expensereport(this.month,this.year);
 }
 ...
 onMonthSelect(event: any) {
   ...
   this.expensereport(month ,year );
 }

 async expensereport(month:string, year:string) {
   ...
   try {
     this.apiService.attendancereport( queryName,employeeId??"",CompanyId??"",month,year).subscribe((response)=>{
       ...
       this.expenseData = data.map((item: any) => {
